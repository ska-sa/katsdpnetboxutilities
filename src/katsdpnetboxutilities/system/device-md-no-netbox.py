...
def main():
    ...
    filename = "{}/index.md".format('docs/docs')
    #device_info = RemoteDeviceInfo(config["device_info"], config["device_name"])
    device_info = RemoteDeviceInfo('http://sdp-services.sdp.kat.ac.za/servers', 'cal1')
    page = DeviceDocument(device_info)
    print(page)
    page.write(filename)
